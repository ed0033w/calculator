...
//
import UIKit
import Darwin//search from internet and discuss with classmate
import CalculatorCore

//just like+-*/ it needs two inputs to calculate, so i try to create a symbol
...
func ^(base: Double,num: Double) -> Double{
    return  pow(base,num)
}

infix operator ^/ : MyPrecedence
func ^/(base: Double,num: Double) -> Double{
    return  pow(base,1.0/num)
}
//symbol created! i can do the ssme step as +-*/
extension Double {
...
class ViewController: UIViewController {
    ...
    @IBAction func plusminusButtonClicked(_ sender: UIButton) {
        let currentNumber = Double(self.displayLabel.text ?? "0")!
        guard currentNumber != 0 else {
            return
        }
        self.displayLabel.text = "\((currentNumber)*(-1))";
    }
    // MARK: - Actions
